    ...
    function createSharedBoard() {
        const newBoardId = Math.random().toString(36).substring(2, 9) + Math.random().toString(36).substring(2, 9);
        joinSharedBoard(newBoardId, true);
    }

    function joinSharedBoard(idToJoin = null, isNew = false) {
        const boardId = idToJoin || document.getElementById('joinBoardInput').value.trim();
        if (!boardId) return;

        currentBoardId = boardId;
        boardRef = db.ref('boards/' + currentBoardId + '/entries');

        // Update UI
        const badge = document.getElementById('boardStatusBadge');
        if (badge) {
            badge.textContent = `Online: ${currentBoardId}`;
            badge.className = 'status-badge online';
            document.getElementById('createBoardBtn').style.display = 'none';
            document.querySelector('.join-container').style.display = 'none';
            document.getElementById('leaveBoardBtn').style.display = 'block';
        }
        
        // Update URL
        const newUrl = new URL(window.location);
        newUrl.searchParams.set('board', currentBoardId);
        window.history.pushState({}, '', newUrl);

        if (isNew) {
            // Push current local entries to start the board
            saveEntries();
        }
    ...
    function leaveSharedBoard() {
        ...
        if (badge) {
            ...
            document.getElementById('joinBoardInput').value = '';
        }
    ...
    function saveEntries() {
        if (currentBoardId && boardRef) {
            boardRef.set(entries).catch(error => {
                console.error("Error saving to Firebase:", error);
                alert("Failed to sync to shared board!");
            });
